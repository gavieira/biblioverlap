...
data_preprocessing <- function(db_list, matching_fields) {
  preprocessed_data <- lapply(db_list, function(db) {
    ...
    db %>%
      ...
      #dplyr::select(dplyr::all_of(unlist(matching_fields)))
      ...
      #dplyr::rowwise() %>%
      #dplyr::mutate(Author = sub(";.*$", "", AU ) ) %>% #Getting only the first author from the 'AU' column; patter removes text after the first ';' separator
